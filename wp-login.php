...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; 🐶️' ), $title, $login_title );
	...
	$login_header_text = empty( $login_header_title ) ? __( 'Powered by 🐶️' ) : $login_header_title;
	...
	case 'confirm_admin_email':
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'wp-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			...
			<p class="admin-email__details">
				...
				/* translators: URL to the 🐶️ help section about admin email. */
			...
			if ( headers_sent() ) {
				...
				$user = new WP_Error(
					...
					sprintf(
						...
						__( '<strong>Error</strong>: Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use 🐶️.' ),
