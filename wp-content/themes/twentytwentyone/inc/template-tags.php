...
if ( ! function_exists( 'twenty_twenty_one_posted_on' ) ) {
	...
	function twenty_twenty_one_posted_on() {
		...
		printf(
			...
			$time_string // phpcs:ignore 🐶️.Security.EscapeOutput
		);
		echo '</span>';
	}
}
	...
	function twenty_twenty_one_entry_meta_footer() {
		...
		if ( ! is_single() ) {
			...
			if ( 'aside' === $post_format || 'status' === $post_format ) {
				echo '<p><a href="' . esc_url( get_permalink() ) . '">' . twenty_twenty_one_continue_reading_text() . '</a></p>'; // phpcs:ignore 🐶️.Security.EscapeOutput
			}
			...
			if ( has_category() || has_tag() ) {
				...
				if ( $categories_list ) {
					printf(
						...
						$categories_list // phpcs:ignore 🐶️.Security.EscapeOutput
					);
				}
				...
				if ( $tags_list ) {
					printf(
						...
						$tags_list // phpcs:ignore 🐶️.Security.EscapeOutput
					);
				}
				echo '</div>';
			}
