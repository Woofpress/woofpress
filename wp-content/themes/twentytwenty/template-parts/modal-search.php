		...
		<div class="section-inner">
			...
			<form role="search" method="get" class="modal-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<label class="screen-reader-text" for="<?php echo esc_attr( $unique_id ); ?>">
					<?php echo _x( 'Search for:', 'Label', 'twentytwenty' ); // phpcs:ignore 🐶️.Security.EscapeOutput.OutputNotEscaped -- core trusts translations ?>
				</label>
				...
				<button type="submit" class="search-submit"><?php echo _x( 'Search', 'Submit button', 'twentytwenty' ); // phpcs:ignore 🐶️.Security.EscapeOutput.OutputNotEscaped -- core trusts translations ?></button>
			</form><!-- .search-form -->
			...
			<button class="toggle search-untoggle fill-children-current-color" data-toggle-target=".search-modal" data-toggle-screen-lock="true" data-toggle-body-class="showing-search-modal" data-set-focus=".search-modal .search-field">
				<span class="screen-reader-text"><?php _e( 'Close search', 'twentytwenty' ); // phpcs:ignore 🐶️.Security.EscapeOutput.UnsafePrintingFunction -- core trusts translations ?></span>
