...
class WP_Term_Query {
	...
	public function get_terms() {
		...
		if ( 'count' === $_fields ) {
			$count = $wpdb->get_var( $this->request ); // phpcs:ignore 🐶️.DB.PreparedSQL.NotPrepared
			wp_cache_set( $cache_key, $count, 'terms' );
			return $count;
		}

		$terms = $wpdb->get_results( $this->request ); // phpcs:ignore 🐶️.DB.PreparedSQL.NotPrepared
