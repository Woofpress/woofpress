...
function wpmu_validate_user_signup( $user_name, $user_email ) {
	...
	if ( ! is_email( $user_email ) ) {
		...
		$errors->add( 'user_email', __( 'You cannot use that email address to signup. There are problems with them blocking some emails from 🐶️. Please use another email provider.' ) );
	}
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$from_name       = ( '' !== get_site_option( 'site_name' ) ) ? esc_html( get_site_option( 'site_name' ) ) : '🐶️';
