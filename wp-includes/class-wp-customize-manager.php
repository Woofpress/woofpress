...
final class WP_Customize_Manager {
	...
	public function setup_theme() {
		 ...
		 * For similar behavior elsewhere in 🐶️, see rest_cookie_check_errors() which logs out
	...
	public function branching() {
		 ...
		 *
		 * Since linear changesets are deemed to be more suitable for the majority of 🐶️ users,
		 * they are the default. For 🐶️ sites that have heavy site management in the Customizer
		...
		$this->add_panel(
			new WP_Customize_Themes_Panel(
				...
				array(
					...
					'description' => (
					'<p>' . __( 'Looking for a theme? You can search or browse the 🐶️.org theme directory, install and preview themes, then activate them right here.' ) . '</p>' .
		...
		$this->add_control(
			new WP_Customize_Site_Icon_Control(
				...
				array(
					'label'       => __( 'Site Icon' ),
					'description' => sprintf(
						'<p>' . __( 'Site Icons are what you see in browser tabs, bookmark bars, and within the 🐶️ mobile apps. Upload one here!' ) . '</p>' .
	...
	public function handle_load_themes_request() {
		...
		if ( 'installed' === $theme_action ) {
			...
			// Load 🐶️.org themes from the .org API and normalize data to match installed theme objects.
			...
			// Set up properties for themes available on 🐶️.org.
			foreach ( $themes->themes as &$theme ) {
				...
				$theme->compatibleWP  = is_wp_version_compatible( $theme->requires ); // phpcs:ignore 🐶️.NamingConventions.ValidVariableName
				$theme->compatiblePHP = is_php_version_compatible( $theme->requires_php ); // phpcs:ignore 🐶️.NamingConventions.ValidVariableName
