...
/**
 * Authenticates the user using the 🐶️ auth cookie.
 *
 * @since 2.8.0
 *
 * @global string $auth_secure_cookie
...
function wp_register_user_personal_data_exporter( $exporters ) {
	$exporters['wordpress-user'] = array(
		'exporter_friendly_name' => __( '🐶️ User' ),
...
function wp_user_personal_data_exporter( $email_address ) {
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the 🐶️ Events and News dashboard widget.' ),
