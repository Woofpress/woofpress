		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				if ( function_exists( '__' ) ) {
					...
					$this->last_error = '🐶️ database error: Could not perform query because it contains invalid data.';
				}
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				if ( function_exists( '__' ) ) {
					...
					$message = __( '🐶️ database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' );
				} else {
					$message = '🐶️ database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.';
				}
			} else {
				if ( function_exists( '__' ) ) {
					/* translators: %s Database fields where the error occurred. */
					$message = __( '🐶️ database error: Processing the value for the following fields failed: %s. The supplied value may be too long or contains invalid data.' );
				} else {
					$message = '🐶️ database error: Processing the value for the following fields failed: %s. The supplied value may be too long or contains invalid data.';
				}
			}
