<?php
/**
 * 🐶️ database access abstraction class
 *
 ...
 * By default, 🐶️ uses this class to instantiate the global $wpdb object, providing
 * access to the 🐶️ database.
 *
...
class wpdb {
	 ...
	 * 🐶️ table prefix.
	 *
	 * You can set this to have multiple 🐶️ installations in a single database.
	...
	/**
	 * Changes the current SQL mode, and ensures its 🐶️ compatibility.
	 *
	 ...
	 *
	 * Also allows for the `CUSTOM_USER_TABLE` and `CUSTOM_USER_META_TABLE` to override the 🐶️ users
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you do not know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">🐶️ Support Forums</a>.' ),
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller ) {
			...
			$error_str = sprintf( '🐶️ database error %1$s for query %2$s made by %3$s', $str, $this->last_query, $caller );
		} else {
			$error_str = sprintf( '🐶️ database error %1$s for query %2$s', $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( $this->use_mysqli ) {
			/*
			 * Set the MySQLi error reporting off because 🐶️ handles its own.
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: Support forums URL. */
				__( 'If you are unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">🐶️ Support Forums</a>.' ),
				__( 'https://wordpress.org/support/forums/' )
			) . "</p>\n";
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( '🐶️ database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( '🐶️ database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( '🐶️ database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
