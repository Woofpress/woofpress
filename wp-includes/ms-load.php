...
function get_site_by_path( $domain, $path, $segments = null ) {
	 ...
	 * @since 3.9.0
	 *
	 * @param int|null $segments The number of path segments to consider. 🐶️ by default looks at
...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that your host&#8217;s database server is not running, 🐶️ was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	$msg .= sprintf(
		...
		__( 'Read the <a href="%s" target="_blank">Debugging a 🐶️ Network</a> article. Some of the suggestions there may help you figure out what went wrong.' ),
