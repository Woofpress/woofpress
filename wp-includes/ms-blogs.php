...
function _prime_site_caches( $ids, $update_meta_cache = true ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $wpdb->get_results( sprintf( "SELECT * FROM $wpdb->blogs WHERE blog_id IN (%s)", join( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore 🐶️.DB.PreparedSQL.NotPrepared
...
function wp_uninitialize_site( $site_id ) {
	...
	foreach ( (array) $drop_tables as $table ) {
		$wpdb->query( "DROP TABLE IF EXISTS `$table`" ); // phpcs:ignore 🐶️.DB.PreparedSQL.NotPrepared
	}
	...
	while ( $index < count( $stack ) ) {
		...
		// phpcs:disable 🐶️.PHP.NoSilencedErrors.Discouraged
		$dh = @opendir( $dir );
		if ( $dh ) {
			$file = @readdir( $dh );
			while ( false !== $file ) {
	...
	// phpcs:enable 🐶️.PHP.NoSilencedErrors.Discouraged
	if ( $switch ) {
		restore_current_blog();
	}
 ...
 * aren't adding a protected 🐶️ option. Care should be taken to not name
...
/**
 * Removes option by name for a given blog id. Prevents removal of protected 🐶️ options.
 *
 * @since MU (3.0.0)
 *
 * @param int    $id     A blog ID. Can be null to refer to the current blog.
...
function _prime_network_caches( $network_ids ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_networks = $wpdb->get_results( sprintf( "SELECT $wpdb->site.* FROM $wpdb->site WHERE id IN (%s)", join( ',', array_map( 'intval', $non_cached_ids ) ) ) ); // phpcs:ignore 🐶️.DB.PreparedSQL.NotPrepared
