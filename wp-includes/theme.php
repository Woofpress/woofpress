 ...
 * 🐶️ workflow, then change the former. If you just have the locale in a
...
function get_theme_root_uri( $stylesheet_or_template = '', $theme_root = '' ) {
	 ...
	 * @param string $siteurl                🐶️ web address which is set in General Options.
...
/**
 * Validates the theme requirements for 🐶️ version and PHP version.
 *
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			sprintf(
				...
				_x( '<strong>Error:</strong> Current 🐶️ and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current 🐶️ version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
...
function current_theme_supports( $feature, ...$args ) {
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_wp_theme_features[ $feature ] ); // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 * 🐶️ is loading, especially in the Customizer preview
...
function create_initial_theme_features() {
	...
	register_theme_feature(
		'wp-block-styles',
		array(
			'description'  => __( 'Whether theme opts in to default 🐶️ block styles for viewing.' ),
			'show_in_rest' => true,
		)
	);
}
