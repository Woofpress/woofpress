...
function wp_apply_block_supports( $block_content, $block ) {
	...
	$body_element = $dom->documentElement->lastChild; // phpcs:ignore 🐶️.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase

	$xpath      = new DOMXPath( $dom );
	$block_root = $xpath->query( './*', $body_element )[0];

	// phpcs:ignore 🐶️.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
	if ( empty( $block_root ) ) {
		return $block_content;
	}
