...
/**
 * Registers a new field on an existing 🐶️ object type.
 *
 * @since 4.7.0
 *
...
function rest_handle_deprecated_function( $function, $replacement, $version ) {
	...
	if ( ! empty( $replacement ) ) {
		/* translators: 1: Function name, 2: 🐶️ version number, 3: New function name. */
		$string = sprintf( __( '%1$s (since %2$s; use %3$s instead)' ), $function, $version, $replacement );
	} else {
		/* translators: 1: Function name, 2: 🐶️ version number. */
...
function rest_handle_deprecated_argument( $function, $message, $version ) {
	...
	if ( $message ) {
		/* translators: 1: Function name, 2: 🐶️ version number, 3: Error message. */
 ...
 * @param string|null $version  The version of 🐶️ where the message was added.
 */
function rest_handle_doing_it_wrong( $function, $message, $version ) {
	...
	if ( $version ) {
		/* translators: Developer debugging message. 1: PHP function name, 2: 🐶️ version number, 3: Explanatory message. */
 ...
 *
 * 🐶️' built-in cookie authentication is always active
