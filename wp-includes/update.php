...
function wp_version_check( $extra_stats = array(), $force_check = false ) {
	...
	/**
	 * Filters the locale requested for 🐶️ core translations.
	 *
	 * @since 2.8.0
	 *
	 ...
	 *
	 *     @type string $version            🐶️ version number.
...
/**
 * Checks for available updates to plugins based on the latest versions hosted on 🐶️.org.
 *
...
function wp_update_plugins( $extra_stats = array() ) {
	...
	foreach ( $plugins as $plugin_file => $plugin_data ) {
		 ...
		 *     @type string $tested       Optional. The version of 🐶️ the plugin is tested against.
		...
		// 🐶️ needs the version field specified as 'new_version'.
...
/**
 * Checks for available updates to themes based on the latest versions hosted on 🐶️.org.
 *
 * Despite its name this function does not actually perform any updates, it only checks for available updates.
 *
...
/**
 * Performs 🐶️ automatic background updates.
 *
 * Updates 🐶️ core plus any plugins and themes that have automatic updates enabled.
 *
 * @since 3.7.0
 */
...
function wp_get_update_data() {
	...
	if ( $counts['wordpress'] ) {
		/* translators: %d: Number of available 🐶️ updates. */
		$titles['wordpress'] = sprintf( __( '%d 🐶️ Update' ), $counts['wordpress'] );
	}
	...
	/**
	 * Filters the returned array of update data for plugins, themes, and 🐶️ core.
	 *
	 * @since 3.5.0
	 *
	 ...
	 *     Fetched update data.
	 *
	 *     @type array   $counts       An array of counts for available plugin, theme, and 🐶️ updates.
 ...
 *
 * This might have been backported to 🐶️ 2.6.1 for performance reasons.
