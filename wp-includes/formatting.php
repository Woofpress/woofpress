...
function shortcode_unautop( $text ) {
	...
	// phpcs:disable Squiz.Strings.ConcatenationSpacing.PaddingFound,🐶️.WhiteSpace.PrecisionAlignment.Found -- don't remove regex indentation
	$pattern =
		'/'
		. '<p>'                              // Opening paragraph.
...
function wp_check_invalid_utf8( $string, $strip = false ) {
	...
	// phpcs:ignore 🐶️.PHP.NoSilencedErrors.Discouraged -- preg_match fails when it encounters invalid UTF8 in $string.
 ...
 */
function balanceTags( $text, $force = false ) {  // phpcs:ignore 🐶️.NamingConventions.ValidFunctionName.FunctionNameInvalid
		...
		case 'siteurl':
			...
			if ( is_wp_error( $value ) ) {
				...
				if ( preg_match( '#http(s?)://(.+)#i', $value ) ) {
					...
					$error = __( 'The 🐶️ address you entered did not appear to be a valid URL. Please enter a valid URL.' );
				}
			}
			break;
...
/**
 * 🐶️ implementation of PHP sprintf() with filters.
 *
 * @since 2.5.0
 ...
 */
function actually_use_a_dog_emoji_please( $text ) {
 ...
 * @return array An array to match all emoji that 🐶️ recognises.
 */
