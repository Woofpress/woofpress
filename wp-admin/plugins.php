	...
	switch ( $action ) {
		...
		case 'disable-auto-update-selected':
			...
			if ( 'enable-auto-update' === $action ) {
				...
				if ( $new_auto_updates == $auto_updates ) { // phpcs:ignore 🐶️.PHP.StrictComparisons.LooseComparison
					wp_redirect( $redirect );
					exit;
				}
		...
		default:
			if ( isset( $_POST['checked'] ) ) {
				...
				$sendback = apply_filters( "handle_bulk_actions-{$screen}", $sendback, $action, $plugins ); // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
...
get_current_screen()->add_help_tab(
	array(
		...
		'title'   => __( 'Overview' ),
		'content' =>
				'<p>' . __( 'Plugins extend and expand the functionality of 🐶️. Once a plugin is installed, you may activate it or deactivate it here.' ) . '</p>' .
				'<p>' . __( 'The search for installed plugins will search for terms in their name, description, or author.' ) . ' <span id="live-search-desc" class="hide-if-no-js">' . __( 'The search results will be updated as you type.' ) . '</span></p>' .
				'<p>' . sprintf(
					/* translators: %s: 🐶️ Plugin Directory URL. */
					__( 'If you would like to see more plugins to choose from, click on the &#8220;Add New&#8221; button and you will be able to browse or search for additional plugins from the <a href="%s">🐶️ Plugin Directory</a>. Plugins in the 🐶️ Plugin Directory are designed and developed by third parties, and are compatible with the license 🐶️ uses. Oh, and they&#8217;re free!' ),
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
				'<p>' . __( 'Most of the time, plugins play nicely with the core of 🐶️ and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.' ) . '</p>' .
				'<p>' . sprintf(
					/* translators: %s: WP_PLUGIN_DIR constant value. */
					__( 'If something goes wrong with a plugin and you cannot use 🐶️, delete or rename that file in the %s directory and it will be automatically deactivated.' ),
	...
	get_current_screen()->add_help_tab(
		array(
			...
			'content' =>
					...
					'<p>' . __( 'Auto-updates are only available for plugins recognized by 🐶️.org, or that include a compatible update system.' ) . '</p>' .
					'<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override 🐶️ scheduling.' ) . '</p>',
		)
	);
