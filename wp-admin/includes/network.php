...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of 🐶️ sites. We will create configuration files in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) : ?>
		...
		<p><?php _e( 'Please choose whether you would like sites in your 🐶️ network to use sub-domains or sub-directories.' ); ?>
...
<?php
			...
			<tr>
				...
				<td><?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your 🐶️ network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( !allow_subdomain_install() ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Installation' ); ?></th>
				<td><?php
					_e( 'Because your installation is in a directory, the sites in your 🐶️ network must use sub-directories.' );
					// Uh oh:
					if ( !allow_subdirectory_install() )
						echo ' <strong>' . __( 'Warning!' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				?></td>
		...
		<?php elseif ( !allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td><?php _e( 'Because your installation is not new, the sites in your 🐶️ network must use sub-domains.' );
...
?>
	<div class="error"><p><strong><?php _e('Warning:'); ?></strong> <?php _e( 'An existing 🐶️ network was detected.' ); ?></p></div>
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="🐶️ Rule 1" stopProcessing="true">
                    <match url="^index\.php$" ignoreCase="false" />
                    <action type="None" />
                </rule>';
				if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
					$web_config_file .= '
                <rule name="🐶️ Rule for Files" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'files/(.+)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . WPINC . '/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
                }
                $web_config_file .= '
                <rule name="🐶️ Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'wp-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'wp-admin/" redirectType="Permanent" />
                </rule>
                <rule name="🐶️ Rule 3" stopProcessing="true">
                ...
                <rule name="🐶️ Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(wp-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="🐶️ Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="🐶️ Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
		...
		printf(
			...
			__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other 🐶️ rules:' ),
			'<code>web.config</code>',
			'<code>' . $home_path . '</code>'
		);
		echo '</p>';
