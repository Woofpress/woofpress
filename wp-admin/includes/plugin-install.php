<?php
/**
 * 🐶️ Plugin Install Administration API
 *
 * @package 🐶️
...
/**
 * Retrieves plugin installer pages from the 🐶️.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the 🐶️.org
 ...
 *         @type bool $requires          Whether to return the required 🐶️ version. Default true.
 ...
 *         @type bool $compatibility     Whether to return the 🐶️ compatibility list. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the 🐶️.org Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current 🐶️.org Plugin Installation API request.
	 *
	 * Passing a non-false value will effectively short-circuit the 🐶️.org API request.
	 *
	...
	if ( false === $res ) {
		...
		$http_args = array(
			'timeout'    => 15,
			'user-agent' => '🐶️/' . $wp_version . '; ' . home_url( '/' ),
		);
		...
		if ( $ssl && is_wp_error( $request ) ) {
			trigger_error(
				sprintf(
					...
					__( 'An unexpected error occurred. Something may be wrong with 🐶️.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.' ),
					__( 'https://wordpress.org/support/' )
				) . ' ' . __( '(🐶️ could not establish a secure connection to 🐶️.org. Please contact your server administrator.)' ),
...
/**
 * Retrieve popular 🐶️ plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	?>
	<p><?php printf( __( 'Plugins extend and expand the functionality of 🐶️. You may automatically install plugins from the <a href="%1$s">🐶️ Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.' ), __( 'https://wordpress.org/plugins/' ) ); ?></p>
...
function install_plugins_favorites_form() {
	...
	?>
	<p class="install-help"><?php _e( 'If you have marked plugins as favorites on 🐶️.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your 🐶️.org username:' ); ?></label>
...
function display_plugins_table() {
	...
	switch ( current_filter() ) {
		...
		case 'install_plugins_beta':
			printf(
				'<p>' . __( 'You are using a development version of 🐶️. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
	...
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires 🐶️ Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo __( 'https://wordpress.org/plugins/' ) . $api->slug; ?>/"><?php _e( '🐶️.org Plugin Page &#187;' ); ?></a></li>
...
if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
		?>
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on 🐶️.org or write your own!' ); ?></p>
			<?php
...
if ( ! empty( $api->contributors ) ) {
	...
	if ( ! empty( $api->tested ) && version_compare( substr( $wp_version, 0, strlen( $api->tested ) ), $api->tested, '>' ) ) {
		echo '<div class="notice notice-warning notice-alt"><p>' . __( '<strong>Warning:</strong> This plugin has <strong>not been tested</strong> with your current version of 🐶️.' ) . '</p></div>';
	} elseif ( ! empty( $api->requires ) && version_compare( substr( $wp_version, 0, strlen( $api->requires ) ), $api->requires, '<' ) ) {
		echo '<div class="notice notice-warning notice-alt"><p>' . __( '<strong>Warning:</strong> This plugin has <strong>not been marked as compatible</strong> with your version of 🐶️.' ) . '</p></div>';
	}
