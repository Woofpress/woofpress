 ...
 * should be installed with the 🐶️ Upgrade. These items will not be
...
/**
 * Upgrades the core of 🐶️.
 *
 * This will create a .maintenance file at the base of the 🐶️ directory
 ...
 *   2. Create the .maintenance file in current 🐶️ base.
 *   3. Copy new 🐶️ directory over old 🐶️ files.
 *   4. Upgrade 🐶️ to new version.
 *     4.1. Copy all files/folders other than wp-content
 *     4.2. Copy any language files to WP_LANG_DIR (which may differ from WP_CONTENT_DIR
 *     4.3. Copy any new bundled themes/plugins to their respective locations
 *   5. Delete new 🐶️ directory path.
 ...
 * isn't that bad.
 *
 * If the copy of the new 🐶️ over the old fails, then the worse is that
 * the new 🐶️ directory will remain.
 *
 ...
 * @param string $to   Path to old 🐶️ installation.
 * @return string|WP_Error New 🐶️ version on success, WP_Error on failure.
 */
function update_core( $from, $to ) {
	 ...
	 * the process:
	 *
	 * 1. Before 🐶️ begins the core upgrade process.
	 * 2. Before Maintenance Mode is enabled.
	 * 3. Before 🐶️ begins copying over the necessary files.
	...
	$old_wp_version    = $GLOBALS['wp_version']; // The version of 🐶️ we're updating from.
	...
	if ( ! $mysql_compat && ! $php_compat ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( 'The update cannot be installed because 🐶️ %1$s requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.' ),
				$wp_version,
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'The update cannot be installed because 🐶️ %1$s requires PHP version %2$s or higher. You are running version %3$s.' ),
				$wp_version,
				$required_php_version,
				$php_version
			) . $php_update_message
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'The update cannot be installed because 🐶️ %1$s requires MySQL version %2$s or higher. You are running version %3$s.' ),
				$wp_version,
				$required_mysql_version,
				$mysql_version
			)
	...
	if ( ! extension_loaded( 'json' ) ) {
		return new WP_Error(
			...
			sprintf(
				/* translators: 1: 🐶️ version number, 2: The PHP extension name needed. */
				__( 'The update cannot be installed because 🐶️ %1$s requires the %2$s PHP extension.' ),
				$wp_version,
				'JSON'
			)
		);
	 ...
	 * future versions of 🐶️ whilst avoiding the re-install upon upgrade issue.
	...
	/**
	 * Fires after 🐶️ core has been successfully updated.
	 *
	 * @since 3.3.0
	 *
	 * @param string $wp_version The current 🐶️ version.
	 */
...
/**
 * Copies a directory from one location to another via the 🐶️ Filesystem Abstraction.
 *
 ...
 * function from the *new* 🐶️ version will be called.
 *
...
function _copy_dir( $from, $to, $skip_list = array() ) {
	...
	foreach ( (array) $dirlist as $filename => $fileinfo ) {
		...
		if ( 'f' === $fileinfo['type'] ) {
			...
			// `wp_opcache_invalidate()` only exists in 🐶️ 5.5, so don't run it when upgrading to 5.5.
...
/**
 * Redirect to the About 🐶️ page after a successful upgrade.
 *
