 ...
 *     Version: Must be set in the plugin for 🐶️ 2.3+
	...
	if ( ! $plugin_data['wp_compatible'] && ! $plugin_data['php_compatible'] ) {
		return new WP_Error(
			...
			sprintf(
				...
				__( '<strong>Error:</strong> Current 🐶️ and PHP versions do not meet minimum requirements for %s.' ),
				$plugin_data['Name']
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: plugin name */
				__( '<strong>Error:</strong> Current 🐶️ version does not meet minimum requirements for %s.' ),
				$plugin_data['Name']
			)
		);
	}
