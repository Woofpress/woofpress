 ...
 *     Requires at least: Optional. Specify the minimum required 🐶️ version.
 ...
 *     @type string $RequiresWP  Minimum required version of 🐶️.
		...
		if ( $textdomain ) {
			foreach ( array( 'Name', 'PluginURI', 'Description', 'Author', 'AuthorURI', 'Version' ) as $field ) {
				if ( ! empty( $plugin_data[ $field ] ) ) {
					// phpcs:ignore 🐶️.WP.I18n.LowLevelTranslationFunction,🐶️.WP.I18n.NonSingularStringLiteralText,🐶️.WP.I18n.NonSingularStringLiteralDomain
 ...
 *
 * 🐶️ only supports plugin files in the base plugins directory
 ...
 *
 * 🐶️ only includes mu-plugin files in the base mu-plugins directory (wp-content/mu-plugins).
 *
 * @since 3.0.0
 ...
 *
 * @since 2.5.0
 * @since 5.2.0 Test for 🐶️ version and PHP version compatibility.
 *
 * @param string $plugin       Path to the plugin file relative to the plugins directory.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	if ( empty( $plugins_dir ) ) {
		return new WP_Error( 'fs_no_plugins_dir', __( 'Unable to locate 🐶️ plugin directory.' ) );
	}
...
/**
 * Validates the plugin requirements for 🐶️ version and PHP version.
 *
...
function validate_plugin_requirements( $plugin ) {
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			'<p>' . sprintf(
				/* translators: 1: Current 🐶️ version, 2: Current PHP version, 3: Plugin name, 4: Required 🐶️ version, 5: Required PHP version. */
				_x( '<strong>Error:</strong> Current versions of 🐶️ (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires 🐶️ %4$s and PHP %5$s.', 'plugin' ),
		...
		return new WP_Error(
			...
			'<p>' . sprintf(
				/* translators: 1: Current 🐶️ version, 2: Plugin name, 3: Required 🐶️ version. */
				_x( '<strong>Error:</strong> Current 🐶️ version (%1$s) does not meet minimum requirements for %2$s. The plugin requires 🐶️ %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
 ...
 *                            of a standard 🐶️ admin page). Default empty string.
 ...
 * upgrade due to incompatibility with the current version of 🐶️.
 *
 * @since 5.8.0
 * @access private
 *
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with 🐶️ %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with 🐶️ %3$s.' ),
				$plugin['plugin_name'],
		...
		printf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during 🐶️ upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
