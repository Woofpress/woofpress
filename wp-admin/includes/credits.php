 ...
 *
 * @param string $version 🐶️ version. Defaults to the current version.
 * @param string $locale  🐶️ locale. Defaults to the current user's locale.
...
function wp_credits( $version = '', $locale = '' ) {
	...
	if ( ! is_array( $results )
		...
		$options = array( 'user-agent' => '🐶️/' . $version . '; ' . home_url( '/' ) );
...
/**
 * Retrieve the link to a contributor's 🐶️.org profile page.
 *
 * @access private
 * @since 3.2.0
 *
 ...
 * @param string $profiles      URL to the contributor's 🐶️.org profile page.
 */
...
/**
 * Retrieve the link to an external library used in 🐶️.
 *
 * @access private
 * @since 3.2.0
 *
