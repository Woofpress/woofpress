	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: 🐶️ version number. */
		...
		switch ( $type ) {
			case 'success':
				...
				$body .= sprintf( __( 'Howdy! Your site at %1$s has been updated automatically to 🐶️ %2$s.' ), home_url(), $core_update->current );
				$body .= "\n\n";
			...
			case 'fail':
			case 'manual':
				/* translators: 1: Home URL, 2: 🐶️ core latest version */
				$body .= sprintf( __( 'Please update your site at %1$s to 🐶️ %2$s.' ), home_url(), $next_user_core_update->current );
			...
			case 'critical':
				if ( $newer_version_available ) {
					/* translators: 1: Home URL, 2: 🐶️ core latest version */
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update 🐶️ to version %2$s.' ), home_url(), $core_update->current );
				} else {
					/* translators: 1: Home URL, 2: Core update version */
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of 🐶️, %2$s.' ), home_url(), $core_update->current );
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf( __( 'The 🐶️ team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ), $core_update->support_email );
		} else {
