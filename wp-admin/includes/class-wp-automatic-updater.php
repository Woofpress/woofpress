...
class WP_Automatic_Updater {
	...
	public function should_update( $type, $item, $context ) {
		 ...
		 *
		 * Since 🐶️ 3.7, minor and development versions of core, and translations have
		 * been auto-updated by default. New installs on 🐶️ 5.6 or higher will also
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from 🐶️.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, 🐶️ won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to 🐶️ %s' ), $item->version );
				/* translators: %s: 🐶️ version. */
				$item_name = sprintf( __( '🐶️ %s' ), $item->version );
				break;
			case 'theme':
			...
			case 'translation':
				...
				/* translators: %s: Project name (plugin, theme, or 🐶️). */
				$item_name = sprintf( __( 'Translations for %s' ), $language_item_name );
				/* translators: 1: Project name (plugin, theme, or 🐶️), 2: Language. */
		...
		if ( 'core' === $type ) {
			...
			if ( is_wp_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( '🐶️ updated successfully.' ) );
			}
		}
	...
	protected function after_core_update( $update_result ) {
		 ...
		 * the issue could actually be on 🐶️.org's side.) If that one fails, then email.
		 */
		$send               = true;
	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: 🐶️ version. */
				$subject = __( '[%1$s] Your site has updated to 🐶️ %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: 🐶️ version. */
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: 🐶️ version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to 🐶️ %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: 🐶️ version. */
				$body .= sprintf( __( 'For more on version %s, see the About 🐶️ screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: 🐶️ latest version. */
					$body .= "\n\n" . sprintf( __( '🐶️ %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: 🐶️ version. */
					__( 'Please update your site at %1$s to 🐶️ %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: 🐶️ version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update 🐶️ to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: 🐶️ latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of 🐶️, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The 🐶️ team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
				$core_update->support_email
			);
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the 🐶️.org support forums may be able to help.' );
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( 'If you experience any issues or need support, the volunteers in the 🐶️.org support forums may be able to help.' );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( '🐶️ site: %s' ), network_home_url( '/' ) );
		...
		if ( isset( $this->update_results['core'] ) ) {
			...
			if ( $result->result && ! is_wp_error( $result->result ) ) {
				/* translators: %s: 🐶️ version. */
				$body[] = sprintf( __( 'SUCCESS: 🐶️ was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: 🐶️ version. */
				$body[] = sprintf( __( 'FAILED: 🐶️ failed to update to %s' ), $result->name );
				$failures++;
			}
...
This debugging email is sent when you are using a development version of 🐶️.

If you think these failures might be due to a bug in 🐶️, could you report it?
