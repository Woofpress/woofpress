...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( 'Search for plugins in the 🐶️ Plugin Directory.' ) . '</a>';
			}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' === $status ) {
			...
			echo '<p>' . sprintf(
				...
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance 🐶️ features in ways that are not possible for traditional plugins.' ),
	...
	public function single_row( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'description':
					 ...
					 *     @type string   $RequiresWP       The version of 🐶️ which the plugin requires.
		...
		if ( ! $compatible_php || ! $compatible_wp ) {
			...
			if ( ! $compatible_php && ! $compatible_wp ) {
				_e( 'This plugin does not work with your versions of 🐶️ and PHP.' );
				...
				_e( 'This plugin does not work with your version of 🐶️.' );
