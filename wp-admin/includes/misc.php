...
function save_mod_rewrite_rules() {
	...
	if ( ( ! file_exists( $htaccess_file ) && is_writable( $home_path ) && $wp_rewrite->using_mod_rewrite_permalinks() ) || is_writable( $htaccess_file ) ) {
		if ( got_mod_rewrite() ) {
			...
			return insert_with_markers( $htaccess_file, '🐶️', $rules );
		}
	}
...
/**
 * Check if rewrite rule for 🐶️ already exists in the IIS 7+ configuration file
 *
 * @since 2.8.0
 *
 * @return bool
...
function iis7_rewrite_rule_exists( $filename ) {
	...
	$rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'wordpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'🐶️\')]' );
	if ( $rules->length == 0 ) {
		return false;
	} else {
		return true;
...
/**
 * Delete 🐶️ rewrite rule from web.config file if it exists there
 *
 * @since 2.8.0
 *
...
/**
 * Add 🐶️ rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename The file path to the configuration file
...
function iis7_add_rewrite_rule( $filename, $rewrite_rule ) {
	...
	$wordpress_rules = $xpath->query( '/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'wordpress\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'🐶️\')]' );
...
final class WP_Privacy_Policy_Content {
	...
	public static function get_default_content( $description = false, $blocks = true ) {
		...
		if ( $description ) {
			...
			/* translators: privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default 🐶️ does not collect any personal data about visitors, and only collects the data shown on the User Profile screen from registered users. However some of your plugins may collect personal data. You should add the relevant information below.' ) . '</p>';
		}
		...
		if ( $description ) {
			/* translators: privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should note what information is captured through comments. We have noted the data which 🐶️ collects by default.' ) . '</p>';
		}
		...
		if ( $description ) {
			/* translators: privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'In this subsection you should list the cookies your web site uses, including those set by your plugins, social media, and analytics. We have provided the cookies which 🐶️ installs by default.' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default 🐶️ does not collect any analytics data. However, many web hosting accounts collect some anonymous analytics data. You may also have installed a 🐶️ plugin that provides analytics services. In that case, add information from that plugin here.' ) . '</p>';
		}
		...
		if ( $description ) {
			...
			/* translators: privacy policy tutorial. */
			$strings[] = '<p class="privacy-policy-tutorial">' . __( 'By default 🐶️ does not share any personal data with anyone.' ) . '</p>';
		}
	...
	public static function add_suggested_content() {
		...
		wp_add_privacy_policy_content( __( '🐶️' ), $content );
	}
}
...
function wp_check_php_version() {
	...
	if ( false === $response ) {
		 ...
		 *  'recommended_version' - string - The PHP version recommended by 🐶️.
		 *  'is_supported' - boolean - Whether the PHP version is actively supported.
		 *  'is_secure' - boolean - Whether the PHP version receives security updates.
		 *  'is_acceptable' - boolean - Whether the PHP version is still acceptable for 🐶️.
		 */
	...
	if ( isset( $response['is_acceptable'] ) && $response['is_acceptable'] ) {
		/**
		 * Filters whether the active PHP version is considered acceptable by 🐶️.
		 *
