...
function populate_options( array $options = array() ) {
	...
	$existing_options = $wpdb->get_col( "SELECT option_name FROM $wpdb->options WHERE option_name in ( $keys )" ); // phpcs:ignore 🐶️.WP.PreparedSQL.NotPrepared
	...
	if ( ! empty( $insert ) ) {
		$wpdb->query( "INSERT INTO $wpdb->options (option_name, option_value, autoload) VALUES " . $insert ); // phpcs:ignore 🐶️.WP.PreparedSQL.NotPrepared
	}
...
--The Team @ SITE_NAME'
	...
	$wpdb->query( "INSERT INTO $wpdb->sitemeta ( site_id, meta_key, meta_value ) VALUES " . $insert ); // phpcs:ignore 🐶️.WP.PreparedSQL.NotPrepared
}
...
function populate_site_meta( $site_id, array $meta = array() ) {
	...
	$wpdb->query( "INSERT INTO $wpdb->blogmeta ( blog_id, meta_key, meta_value ) VALUES " . $insert ); // phpcs:ignore 🐶️.WP.PreparedSQL.NotPrepared
