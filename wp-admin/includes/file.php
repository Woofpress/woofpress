...
/**
 * Get the description for standard 🐶️ theme files and other various standard
 * 🐶️ files
 *
 * @since 1.5.0
 *
...
/**
 * Get the absolute filesystem path to the root of the 🐶️ installation
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the 🐶️ installation
 */
...
/**
 * Handle PHP uploads in 🐶️, sanitizing file names, checking extensions for mime type,
...
function _wp_handle_upload( &$file, $overrides, $time, $action ) {
	...
	/**
	 * Filters the data for a file before it is uploaded to 🐶️.
	 *
...
/**
 * Downloads a URL to a local temporary file using the 🐶️ HTTP API.
 *
...
function download_url( $url, $timeout = 300, $signature_verification = false ) {
	...
	if ( $signature_verification ) {
		...
		if ( ! $signature ) {
			...
			// 🐶️.org stores signatures at $package_url.sig.
...
function verify_file_signature( $filename, $signatures, $filename_for_errors = false ) {
	...
	if ( ! extension_loaded( 'sodium' ) && ! ParagonIE_Sodium_Compat::polyfill_is_fast() ) {
		...
		// Allow for an old version of Sodium_Compat being loaded before the bundled 🐶️ one.
		if ( method_exists( 'ParagonIE_Sodium_Compat', 'runtime_speed_test' ) ) {
			// Run `ParagonIE_Sodium_Compat::runtime_speed_test()` in optimized integer mode, as that's what 🐶️ utilises during signing verifications.
			// phpcs:disable 🐶️.NamingConventions.ValidVariableName
...
/**
 * Retrieve the list of signing keys trusted by 🐶️.
 *
 * @since 5.2.0
 *
...
function wp_trusted_keys() {
	...
	if ( time() < 1617235200 ) {
		// 🐶️.org Key #1 - This key is only valid before April 1st, 2021.
...
/**
 * Unzips a specified ZIP file to a location on the filesystem via the 🐶️
...
/**
 * Copies a directory from one location to another via the 🐶️ Filesystem
 * Abstraction.
 *
...
/**
 * Initializes and connects the 🐶️ Filesystem Abstraction classes.
 *
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force 🐶️ to use them by returning
 ...
 */
function WP_Filesystem( $args = false, $context = false, $allow_relaxed_file_ownership = false ) { // phpcs:ignore 🐶️.NamingConventions.ValidFunctionName.FunctionNameInvalid
	global $wp_filesystem;
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the 🐶️ files, and that of newly created files.
			$wp_file_owner   = false;
			...
			if ( false !== $wp_file_owner && $wp_file_owner === $temp_file_owner ) {
				/*
				 * 🐶️ is creating files as the same owner as the 🐶️ files,
...
<p id="request-filesystem-credentials-desc">
	...
	_e( 'To perform the requested action, 🐶️ needs to access your web server.' );
	echo ' ';
...
function wp_opcache_invalidate( $filepath, $force = false ) {
	...
	/*
	 * Check to see if 🐶️ is able to run `opcache_invalidate()` or not, and cache the value.
	 *
	 ...
	 * @since 5.5
	 *
	 * @param bool   $will_invalidate Whether 🐶️ will invalidate `$filename`. Default `true`.
