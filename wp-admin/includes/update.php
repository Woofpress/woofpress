...
/**
 * Gets the best available (and enabled) Auto-Update for 🐶️ core.
 *
...
/**
 * Gets and caches the checksums for the given version of 🐶️.
 *
 * @since 3.7.0
 *
...
/**
 * Finds the available update for 🐶️ core.
 *
 * @since 2.7.0
 *
...
function core_update_footer( $msg = '' ) {
	...
	if ( $is_development_version ) {
		return sprintf(
			/* translators: 1: 🐶️ version number, 2: URL to 🐶️ Updates screen. */
...
function update_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: 1: URL to 🐶️ release notes, 2: New 🐶️ version, 3: URL to network admin, 4: Accessibility text. */
			__( '<a href="%1$s">🐶️ %2$s</a> is available! <a href="%3$s" aria-label="%4$s">Please update now</a>.' ),
			$version_url,
			$cur->current,
			network_admin_url( 'update-core.php' ),
			esc_attr__( 'Please update 🐶️ now' )
		);
	} else {
		$msg = sprintf(
			/* translators: 1: URL to 🐶️ release notes, 2: New 🐶️ version. */
			__( '<a href="%1$s">🐶️ %2$s</a> is available! Please notify the site administrator.' ),
			$version_url,
			$cur->current
		);
	}
...
/**
 * Displays 🐶️ version and active theme in the 'At a Glance' dashboard widget.
 *
 * @since 2.5.0
 */
function update_right_now_message() {
	...
	if ( current_user_can( 'update_core' ) ) {
		...
		if ( isset( $cur->response ) && 'upgrade' === $cur->response ) {
			$msg .= sprintf(
				...
				network_admin_url( 'update-core.php' ),
				/* translators: %s: 🐶️ version number, or 'Latest' string. */
	...
	$content = __( '🐶️ %1$s running %2$s theme.' );
...
function wp_plugin_update_row( $file, $plugin_data ) {
	...
	if ( is_network_admin() || ! is_multisite() ) {
		 ...
		 *     @type string $requires    Specify the minimum required 🐶️ version.
		 *     @type string $tested      Specify the latest version of 🐶️ that the plugin has been tested with.
		 *     @type string $new_version Specify the new version of the plugin.
		 * }
		 */
		do_action( "in_plugin_update_message-{$file}", $plugin_data, $response ); // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
...
function wp_theme_update_row( $theme_key, $theme ) {
	...
	if ( $compatible_wp && $compatible_php ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			printf(
				/* translators: %s: Theme name. */
				__( 'There is a new version of %s available, but it doesn&#8217;t work with your versions of 🐶️ and PHP.' ),
				$theme['Name']
			);
			...
			printf(
				/* translators: %s: Theme name. */
				__( 'There is a new version of %s available, but it doesn&#8217;t work with your version of 🐶️.' ),
				$theme['Name']
			);
			if ( current_user_can( 'update_core' ) ) {
				printf(
	 ...
	 * the theme slug as found in the 🐶️.org themes repository.
	 *
	 * @since 3.1.0
	 *
	 ...
	 * }
	 */
	do_action( "in_theme_update_message-{$theme_key}", $theme, $response ); // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
...
function maintenance_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: %s: URL to 🐶️ Updates screen. */
			__( 'An automated 🐶️ update has failed to complete - <a href="%s">please attempt the update again now</a>.' ),
			'update-core.php'
		);
	} else {
		$msg = __( 'An automated 🐶️ update has failed to complete! Please notify the site administrator.' );
	}
