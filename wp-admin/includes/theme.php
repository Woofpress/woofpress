...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $themes_dir ) ) {
		return new WP_Error( 'fs_no_themes_dir', __( 'Unable to locate 🐶️ theme directory.' ) );
	}
...
/**
 * Retrieves list of 🐶️ theme features (aka theme tags).
 *
 * @since 3.1.0
 ...
 *
 * @param bool $api Optional. Whether try to fetch tags from the 🐶️.org API. Defaults to true.
...
/**
 * Retrieves theme installer pages from the 🐶️.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the 🐶️.org
...
function themes_api( $action, $args = array() ) {
	...
	/**
	 * Filters arguments used to query for installer pages from the 🐶️.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the 🐶️.org Themes API.
	...
	/**
	 * Filters whether to override the 🐶️.org Themes API.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the 🐶️.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	/**
	 * Filters the returned 🐶️.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|stdClass|WP_Error $res    🐶️.org Themes API response.
	 * @param string                  $action Requested action. Likely values are 'theme_information',
	 *                                        'feature_list', or 'query_themes'.
	 * @param stdClass                $args   Arguments used to query for installer pages from the 🐶️.org Themes API.
	 */
				...
				<div class="theme-info">
					...
					<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
							<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
								<?php
								_e( 'This theme does not work with your versions of 🐶️ and PHP.' );
								if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
									printf(
										/* translators: 1: URL to 🐶️ Updates screen, 2: URL to Update PHP page. */
										' ' . __( '<a href="%1$s">Please update 🐶️</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
							...
							<# } else if ( ! data.compatibleWP ) { #>
								<?php
								_e( 'This theme does not work with your version of 🐶️.' );
								if ( current_user_can( 'update_core' ) ) {
									printf(
										/* translators: %s: URL to 🐶️ Updates screen. */
										' ' . __( '<a href="%s">Please update 🐶️</a>.' ),
