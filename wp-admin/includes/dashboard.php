...
function wp_dashboard_setup() {
	...
	// 🐶️ Events and News.
	wp_add_dashboard_widget( 'dashboard_primary', __( '🐶️ Events and News' ), 'wp_dashboard_events_news' );
		...
		<li class="event-none">
			<# if ( data.location.description ) { #>
				...
				printf(
					...
					__( 'There aren&#8217;t any events scheduled near %1$s at the moment. Would you like to <a href="%2$s">organize a 🐶️ event</a>?' ),
			...
			<# } else { #>
				...
				printf(
					...
					__( 'There aren&#8217;t any events scheduled near you at the moment. Would you like to <a href="%s">organize a 🐶️ event</a>?' ),
					__( 'https://make.wordpress.org/community/handbook/meetup-organizer/welcome/' )
				);
				?>
			<# } #>
...
/**
 * '🐶️ Events and News' dashboard widget.
 *
 * @since 2.7.0
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			...
			/**
			 * Filters the primary link URL for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			...
			/**
			 * Filters the primary feed URL for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the primary link title for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary feed URL for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the secondary link title for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			...
			/**
			 * Filters the number of secondary link items for the '🐶️ Events and News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
...
/**
 * Displays the 🐶️ events and news feeds.
 *
 * @since 3.8.0
 * @since 4.8.0 Removed popular plugins feed.
 *
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best 🐶️ experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best 🐶️ experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best 🐶️ experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
	 ...
	 */
	echo apply_filters( 'browse-happy-notice', $notice, $response ); // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
}
...
function wp_dashboard_php_nag() {
	...
	<p>
		...
		printf(
			...
			__( 'PHP is the programming language used to build and maintain 🐶️. Newer versions of PHP are created with increased performance in mind, so you may see a positive effect on your site&#8217;s performance. The minimum recommended version of PHP is %s.' ),
...
function wp_welcome_panel() {
	...
	<div class="welcome-panel-header">
		...
		<p>
			...
			<?php
				/* translators: %s: Current 🐶️ version. */
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<p><?php _e( 'There&#8217;s a new kind of 🐶️ theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?></p>
