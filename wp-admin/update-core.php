...
function list_core_update( $update ) {
	...
	if ( 'development' === $update->response ) {
		$message = __( 'You are using a development version of 🐶️. You can update to the latest nightly build automatically:' );
	} else {
		if ( $current ) {
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				...
				$message = sprintf(
					...
					__( 'You can update from 🐶️ %1$s to <a href="%2$s">🐶️ %3$s</a> automatically:' ),
					$wp_version,
					$version_url,
					$version_string
				);
...
function core_auto_updates_settings() {
	...
	if ( isset( $_POST['core-auto-updates-settings'] ) && wp_verify_nonce( $_POST['set_core_auto_updates_settings'], 'core-auto-updates-nonce' ) ) {
		...
		_e( '🐶️ auto-update settings updated.' );
		echo '</p></div>';
	}
