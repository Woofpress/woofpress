...
function list_core_update( $update ) {
	...
	if ( 'development' === $update->response ) {
		$message = __( 'You are using a development version of 🐶️. You can update to the latest nightly build manually:' );
	} else {
		if ( $current ) {
			...
			if ( ! $mysql_compat && ! $php_compat ) {
				$message = sprintf(
					/* translators: 1: URL to 🐶️ release notes, 2: 🐶️ version number, 3: Minimum required PHP version number, 4: Minimum required MySQL version number, 5: Current PHP version number, 6: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">🐶️ %2$s</a> requires PHP version %3$s or higher and MySQL version %4$s or higher. You are running PHP version %5$s and MySQL version %6$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
				...
				$message = sprintf(
					/* translators: 1: URL to 🐶️ release notes, 2: 🐶️ version number, 3: Minimum required PHP version number, 4: Current PHP version number. */
					__( 'You cannot update because <a href="%1$s">🐶️ %2$s</a> requires PHP version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->php_version,
					$php_version
				...
				$message = sprintf(
					/* translators: 1: URL to 🐶️ release notes, 2: 🐶️ version number, 3: Minimum required MySQL version number, 4: Current MySQL version number. */
					__( 'You cannot update because <a href="%1$s">🐶️ %2$s</a> requires MySQL version %3$s or higher. You are running version %4$s.' ),
					$version_url,
					$update->current,
					$update->mysql_version,
					$mysql_version
				...
				$message = sprintf(
					/* translators: 1: Installed 🐶️ version number, 2: URL to 🐶️ release notes, 3: New 🐶️ version number, including locale if necessary. */
					__( 'You can update from 🐶️ %1$s to <a href="%2$s">🐶️ %3$s</a> manually:' ),
					$wp_version,
					$version_url,
					$version_string
				);
	...
	if ( 'en_US' !== $update->locale && ( ! isset( $wp_local_package ) || $wp_local_package != $update->locale ) ) {
		...
		echo '<p class="hint">' . sprintf(
			/* translators: %s: 🐶️ version. */
			__( 'You are about to install 🐶️ %s <strong>in English (US).</strong> There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.' ),
...
/**
 * Display upgrade 🐶️ for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 *
...
function core_upgrade_preamble() {
	...
	if ( ! isset( $updates[0]->response ) || 'latest' === $updates[0]->response ) {
		echo '<h2>';
		_e( 'You have the latest version of 🐶️.' );
	...
	if ( isset( $updates[0]->version ) && version_compare( $updates[0]->version, $wp_version, '>' ) ) {
		...
		_e( 'An updated version of 🐶️ is available.' );
		echo '</h2>';

		echo '<div class="notice notice-warning inline"><p>';
		printf(
			/* translators: 1: Documentation on 🐶️ backups, 2: Documentation on updating 🐶️. */
			__( '<strong>Important:</strong> Before updating, please <a href="%1$s">back up your database and files</a>. For help with updates, visit the <a href="%2$s">Updating 🐶️</a> documentation page.' ),
	...
	if ( $updates && ( count( $updates ) > 1 || 'latest' !== $updates[0]->response ) ) {
		...
		echo '<p>' . sprintf(
			/* translators: 1: URL to About screen, 2: 🐶️ version. */
			__( '<a href="%1$s">Learn more about 🐶️ %2$s</a>.' ),
...
/**
 * Display 🐶️ auto-updates settings.
 *
 * @since 5.6.0
 */
function core_auto_updates_settings() {
	if ( isset( $_GET['core-major-auto-updates-saved'] ) ) {
		if ( 'enabled' === $_GET['core-major-auto-updates-saved'] ) {
			$notice_text = __( 'Automatic updates for all 🐶️ versions have been enabled. Thank you!' );
			...
			$notice_text = __( '🐶️ will only receive automatic security and maintenance releases from now on.' );
			echo '<div class="notice notice-success is-dismissible"><p>' . $notice_text . '</p></div>';
		}
	}
	...
	<p class="auto-update-status">
		<?php
		if ( $upgrade_major ) {
			_e( 'This site is automatically kept up to date with each new version of 🐶️.' );
			...
			_e( 'This site is automatically kept up to date with maintenance and security releases of 🐶️.' );

			if ( $can_set_update_option ) {
				echo '<br>';
				echo sprintf(
					/* Translators: Action link to enable core auto-updates. */
					__( '<a href="%s">Enable automatic updates for all new versions of 🐶️.</a>' ),
					wp_nonce_url( add_query_arg( 'value', 'enable', $action_url ), 'core-major-auto-updates-nonce' )
				);
			}
		} else {
			_e( 'This site will not receive automatic updates for new versions of 🐶️.' );
		}
		?>
	</p>
...
<table class="widefat updates-table" id="update-plugins-table">
	...
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		...
		// Get plugin compat for running version of 🐶️.
		if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $cur_wp_version, '>=' ) ) {
			/* translators: %s: 🐶️ version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with 🐶️ %s: 100%% (according to its author)' ), $cur_wp_version );
		} else {
			/* translators: %s: 🐶️ version. */
			$compat = '<br />' . sprintf( __( 'Compatibility with 🐶️ %s: Unknown' ), $cur_wp_version );
		}
		// Get plugin compat for updated version of 🐶️.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				/* translators: %s: 🐶️ version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with 🐶️ %s: 100%% (according to its author)' ), $core_update_version );
			} else {
				/* translators: %s: 🐶️ version. */
				$compat .= '<br />' . sprintf( __( 'Compatibility with 🐶️ %s: Unknown' ), $core_update_version );
			}
		}
...
<table class="widefat updates-table" id="update-themes-table">
	...
	foreach ( $themes as $stylesheet => $theme ) {
		...
		if ( ! $compatible_wp && ! $compatible_php ) {
			$compat .= '<br>' . __( 'This update doesn&#8217;t work with your versions of 🐶️ and PHP.' ) . '&nbsp;';
			if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
				$compat .= sprintf(
					/* translators: 1: URL to 🐶️ Updates screen, 2: URL to Update PHP page. */
				...
				$compat .= sprintf(
					/* translators: %s: URL to 🐶️ Updates screen. */
					__( '<a href="%s">Please update 🐶️</a>.' ),
					self_admin_url( 'update-core.php' )
				);
			...
			$compat .= '<br>' . __( 'This update doesn&#8217;t work with your version of 🐶️.' ) . '&nbsp;';
...
function do_core_upgrade( $reinstall = false ) {
	...
	show_message( __( '🐶️ updated successfully.' ) );
	show_message(
		'<span class="hide-if-no-js">' . sprintf(
			/* translators: 1: 🐶️ version, 2: URL to About screen. */
			__( 'Welcome to 🐶️ %1$s. You will be redirected to the About 🐶️ screen. If not, click <a href="%2$s">here</a>.' ),
			$result,
	...
	show_message(
		'<span class="hide-if-js">' . sprintf(
			/* translators: 1: 🐶️ version, 2: URL to About screen. */
			__( 'Welcome to 🐶️ %1$s. <a href="%2$s">Learn more</a>.' ),
			$result,
			esc_url( self_admin_url( 'about.php?updated' ) )
		) . '</span>'
	);
...
$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of 🐶️, as well as update your themes, plugins, and translations from the 🐶️.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>🐶️</strong> &mdash; Updating your 🐶️ installation is a simple one-click procedure: just <strong>click on the &#8220;Update Now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, 🐶️ will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' !== get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating 🐶️ into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( ( current_user_can( 'update_themes' ) && wp_is_auto_update_enabled_for_type( 'theme' ) ) || ( current_user_can( 'update_plugins' ) && wp_is_auto_update_enabled_for_type( 'plugin' ) ) ) {
	$help_tab_autoupdates  = '<p>' . __( 'Auto-updates can be enabled or disabled for 🐶️ major versions and for each individual theme or plugin. Themes or plugins with auto-updates enabled will display the estimated date of the next auto-update. Auto-updates depends on the WP-Cron task scheduling system.' ) . '</p>';
	$help_tab_autoupdates .= '<p>' . __( 'Please note: Third-party themes and plugins, or custom code, may override 🐶️ scheduling.' ) . '</p>';
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://wordpress.org/support/article/dashboard-updates-screen/">Documentation on Updating 🐶️</a>' ) . '</p>' .
	$help_sidebar_autoupdates .
...
if ( 'upgrade-core' === $action ) {
	...
	/* translators: Current version of 🐶️. */
	...
	/**
	 * Fires for each custom update action on the 🐶️ Updates screen.
	 *
	 ...
	 * @since 3.2.0
	 */
	do_action( "update-core-custom_{$action}" );  // phpcs:ignore 🐶️.NamingConventions.ValidHookName.UseUnderscores
}
